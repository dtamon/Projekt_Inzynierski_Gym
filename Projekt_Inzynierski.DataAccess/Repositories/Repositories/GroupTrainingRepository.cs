...
namespace Projekt_Inzynierski.DataAccess.Repositories.Repositories
{
    public class GroupTrainingRepository : IGroupTrainingRepository
    {
        ...
        {
            return await _context.GroupTraining.Include(x => x.Trainers).ThenInclude(x => x.Specializations).Include(x => x.Trainer).Include(x => x.Clients).Where(x => x.StartDate > DateTime.Now).ToListAsync();
        }
        ...
        public async Task<ICollection<GroupTraining>> GetTrainingsByTrainerId(int id)
        {
            ...
            return await _context.GroupTraining.Include(x => x.Trainers).ThenInclude(x => x.Specializations).Include(x => x.Trainer).Include(x => x.Clients).Where(x => x.TrainerId == id || x.Trainers.Contains(trainer)).ToListAsync();
        }
        public async Task<ICollection<GroupTraining>> GetTrainingsByClientId(int id)
        {
            ...
            return await _context.GroupTraining.Include(x => x.Trainers).ThenInclude(x => x.Specializations).Include(x => x.Trainer).Include(x => x.Clients).Where(x => x.Clients.Contains(client)).ToListAsync();
        }
